...
app.post('/sertifika', async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ mesaj: 'Sertifika olusturuldu', sertifika_id: r.insertId });
  } catch (e) {
    await conn.rollback();
    if (e.errno === 1062)
      res.status(409).json({ hata: 'Bu seri numarasi zaten kayitli' });
    else if (e.errno === 1452)
      res.status(400).json({ hata: 'Boyle bir parti yok' });
    else if (e.errno === 3819)
      res.status(400).json({ hata: 'Veri kurali ihlali' });
...
app.listen(3000, () => console.log('✓ HydroCert API http://localhost:3000'));
